...
NSDictionary *dictionaryFromPlistData(NSData *data)
{
    ...
    if (!dictionaryParsed)
    {
        if (error)
        {
			NSLog(@"[IAP] Error parsing dictionary from plist: %@", error.localizedDescription);
		}
        return nil;
    }
...
NSDictionary *dictionaryFromJSONData(NSData *data)
{
    ...
    if (!dictionaryParsed)
    {
        if (error)
        {
			NSLog(@"[IAP] Error parsing dictionary from JSON: %@", error.localizedDescription);
        }
        return nil;
    }
    return dictionaryParsed;
...
BOOL NS_INLINE doTransactionDetailsMatchPurchaseInfo(SKPaymentTransaction *transaction, NSDictionary *purchaseInfoDict);
BOOL NS_INLINE isTransactionIdUnique(NSString *transactionId);
// BOOL NS_INLINE checkReceiptSecurity(NSString *purchase_info_string, NSString *signature_string, CFDateRef purchaseDate);
...
/**
 * @brief 这个方法必须在交易状态变为：交易完成（SKPaymentTransactionStatePurchased）
 *        或者购买恢复（SKPaymentTransactionStateRestored）后，立马调用
 * @param transaction 交易信息（包含收据）
 */
BOOL GluVerifyPurchase(SKPaymentTransaction *transaction)
{
    return isTransactionAndItsReceiptValid(transaction);
}
...
/**
 * @brief 检测收据证明。如果检查通过，则同时证明这是某个合法交易。
 *        备注：如果之前没有见过，则在稍后收据验证后，解码并保存从该收据中取得购买信息。
...
BOOL isTransactionAndItsReceiptValid(SKPaymentTransaction *transaction)
{
    ...
    NSDictionary *receiptDict       = dictionaryFromPlistData(transaction.transactionReceipt);
    NSString *transactionPurchaseInfo = [receiptDict objectForKey:@"purchase-info"];
    NSString *decodedPurchaseInfo   = [[[NSString alloc] initWithData:GluDecodeBase64(transactionPurchaseInfo, nil) encoding:NSASCIIStringEncoding] autorelease];
    NSDictionary *purchaseInfoDict  = dictionaryFromPlistData([decodedPurchaseInfo dataUsingEncoding:NSUTF8StringEncoding]);
...
/**
 * @brief 验证交易与购买信息是否相匹配
 * @param transaction 交易信息
 * @param purchaseInfoDict 购买信息
 */
BOOL NS_INLINE doTransactionDetailsMatchPurchaseInfo(SKPaymentTransaction *transaction, NSDictionary *purchaseInfoDict)
{
...
//    
//    NSInteger purchase_info_length;
//    uint8_t *purchase_info_bytes = (unsigned char *)[GluDecodeBase64(purchase_info_string, &purchase_info_length) bytes];
//    
//    NSInteger signature_length;
//    uint8_t *signature_bytes = (unsigned char *)[GluDecodeBase64(signature_string, &signature_length) bytes];
//    
...
static inline void encodeBlock(unsigned char din[3], unsigned char dout[5], int len)
{
    static const char tab64[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
    
    dout[0] = tab64[ din[0] >> 2 ];
    dout[1] = tab64[ ((din[0] & 0x03) << 4) | ((din[1] & 0xf0) >> 4) ];
    dout[2] = (unsigned char) (len > 1 ? tab64[ ((din[1] & 0x0f) << 2) | ((din[2] & 0xc0) >> 6) ] : '=');
    dout[3] = (unsigned char) (len > 2 ? tab64[ din[2] & 0x3f ] : '=');
}

NSString *GluEncodeBase64(const uint8_t *input, NSInteger length)
{
    unsigned char dout[5] = {0, 0, 0, 0, 0};
    unsigned char din[3];
    NSMutableString * result = [NSMutableString stringWithCapacity:(length / 3 + 1) * 4];
    while (length > 0)
    {
        din[0] = input[0];
        din[1] = length > 1 ? input[1] : 0;
        din[2] = length > 2 ? input[2] : 0;
        encodeBlock(din, dout, length);
        [result appendFormat:@"%s", dout];
        input += 3; length -= 3;
    }
    return result;
}

static inline int decodeblock(const char * in, unsigned char out[3] )
{   
    static const unsigned char tab64[] = {
    ...
    if(in[ 0 ] != '=')
    {
        out[ 0 ] = (unsigned char ) (tab64[in[0] - 43] << 2);
        if(in[ 1 ] != '=')
        {
            len++;
            out[ 0 ] |= (unsigned char ) (tab64[in[1] - 43] >> 4);
            out[ 1 ] = (unsigned char ) (tab64[in[1] - 43] << 4);
            if(in[ 2 ] != '=')
            {
                len++;
                out[ 1 ] |= (unsigned char ) (tab64[in[2] - 43] >> 2);
                out[ 2 ] = (unsigned char ) ((tab64[in[2] - 43] << 6) & 0xc0);
                if(in[ 3 ] != '=')
                {
                    len++;
                    out[ 2 ] |= (unsigned char ) (tab64[in[3] - 43]);
                }
            }
        }
    }
...
NSData *GluDecodeBase64(NSString *input, NSInteger *length)
{
	NSMutableData * result = [NSMutableData data];
	if (input != nil) {
		unsigned char dout[3];
		const char * instr = [input cStringUsingEncoding:NSASCIIStringEncoding];
		if(length != nil)
		{
			*length = 0;
		}
		while(*instr != 0)
		{
			int l = decodeblock(instr, dout);
			[result appendBytes:dout length:l];
			instr += 4;
			if(length != nil)
			{
				*length += l;
