...
namespace Upload {
	...
	public delegate void OnFailed(Uploader iUploader, UploadItem iTargetInfo, int iRetries, List<ErrorDetail> iErrors);
	...
	/// <param name="iRetries">剩余重试次数.</param>
	public delegate void OnSuccessed(Uploader iUploader, UploadItem iTargetInfo, int iRetries);
		...
		public IEnumerator UploadFile() {
			...
			if (this._target != null) {
				...
				_bundleThread = new Thread ( delegate() {
					...
					while (this.Retries >0) {
						...
						// Bundle文件
						if(false == this.UpLoadFileToServer()) {
							--this.Retries;
							continue;
						} 
					}
			...
			if (TRunState.OK != this._state) {
				if (this._onFailed != null) {
					this._onFailed (this, this._target, this.Retries, this._errors);
				}
			} else {
