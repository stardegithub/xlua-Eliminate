...
namespace XLua
{
    public partial class LuaTable : LuaBase
    {
        ...
        public bool TryGet<TKey>(TKey key, Type type_of_value, out object value)
        {
#if THREAD_SAFE || HOTFIX_ENABLE
            lock (luaEnv.luaEnvLock)
            {
            ...
            try
            {
                isAssignable = translator.Assignable(L, -1, type_of_value);
                if (isAssignable)
                {
                    translator.Get(L, -1, out value);
                }
        ...
        public void TryForEach<TKey, TValue>(Action<TKey, TValue, bool> action)
        {
#if THREAD_SAFE || HOTFIX_ENABLE
            lock (luaEnv.luaEnvLock)
            {
            ...
            try
            {
                ...
                while (LuaAPI.lua_next(L, -2) != 0)
                {
                    if (translator.Assignable<TKey>(L, -2))
                    {
                        bool isAssignable = translator.Assignable<TValue>(L, -1);
                        if (isAssignable)
                        {
                            TKey key;
                            TValue val;
                        ...
                        {
                            action(default(TKey), default(TValue), isAssignable);
                        }
                    }
                    LuaAPI.lua_pop(L, 1);
                }
