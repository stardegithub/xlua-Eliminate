...
# 导入外部指定宏
echo "$UNITY_PATH -projectPath $PROJECT_PATH -executeMethod CommandLine.DefinesSetting.AddIOSDefines -projectName NFF -debug -defines LOCALIZATION_CN -batchmode -quit"
$UNITY_PATH -projectPath $PROJECT_PATH -executeMethod CommandLine.DefinesSetting.AddIOSDefines -projectName NFF -debug -defines LOCALIZATION_CN -batchmode -quit

# 将unity导出成xcode工程
# 前台运行
# echo "$UNITY_PATH -projectPath $PROJECT_PATH -executeMethod CommandLine.ProjectBuild.ExportXcodeProject -projectName ${XCODE_PROJECT_NAME} -debug -quit"
# $UNITY_PATH -projectPath $PROJECT_PATH -executeMethod CommandLine.ProjectBuild.ExportXcodeProject -projectName ${XCODE_PROJECT_NAME} -debug -quit
# 后台运行
echo "$UNITY_PATH -projectPath $PROJECT_PATH -executeMethod CommandLine.ProjectBuild.ExportXcodeProject -projectName ${XCODE_PROJECT_NAME} -debug -batchmode -quit "
$UNITY_PATH -projectPath $PROJECT_PATH -executeMethod CommandLine.ProjectBuild.ExportXcodeProject -projectName ${XCODE_PROJECT_NAME} -debug -batchmode -quit 
